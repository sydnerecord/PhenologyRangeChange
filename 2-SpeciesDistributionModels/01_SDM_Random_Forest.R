...
#This script creates projected models for phenology and range, two time periods, across species 
...
#Presets 
#########
#This is for super computing -- can remove if gives issues on local device (i.e., laptop)
...
#Directories
#############
#IMPORTANT to set a working directory for biomod2 to pull dependencies
#Output directory -- can change file name  
em = "SDM_output" #output file for results
...
#Data
######
#load phenology & range object files 
...
#See if there are different lengths in the number of species between time periods (chose phenology since those had less species than the range dataset, overall)
sp = intersect(names(phe.h[3:length(phe.h)]), names(phe.c[3:length(phe.c)]))

#Climate rasters 
#1. List file pathways
tif_files_h = list.files("sp_ranges_clim", #Folder directory containing climate data
                         pattern = "^ClimRastS_H_cropped_.*\\.tif$", full.names = TRUE) #Adjust naming convention to your climate data or match
...
species_names <- gsub("ClimRastS_H_cropped_", "", species_names)  #Adjust naming convention to your climate data or match
species_names <- gsub("_", " ", species_names)
#4. Set list labels to species names 
sp.clim.h <- setNames(lapply(tif_files_h, rast), species_names)
#---
#1. List file pathways
tif_files_c = list.files("sp_ranges_clim", #Folder directory containing climate data
                         pattern = "^ClimRastS_C_cropped_.*\\.tif$", full.names = TRUE) #Adjust naming convention to your climate data or match
#2. Read them as rasters
rasters_c <- lapply(tif_files_c, rast)
#3. Pull species names from file pathways
species_names <- tools::file_path_sans_ext(basename(tif_files_c))
species_names <- gsub("ClimRastS_C_cropped_", "", species_names)  #Adjust naming convention to your climate data or match
species_names <- gsub("_", " ", species_names)
#4. Set list labels to species names
...
#############
#Function inputs 
#df (data frame): phe.h, phe.c, rang.h, rang.c
#sp (vector, data frame): species
#time: H, C
#type: Phenology, Range
#e.g., df = phe.h; time = "H"; type= "Phenology"; sp = some vector of species

em.fun = function(df, sp.clim, sp, time, type){
  #Setting working directory to where all default SDM outputs are saved
  #We need to set this because biomod2 will automatically look for supporting files that are in the folder throughout the modeling 
  setwd(em) 
  
  #Check species climate raster 
  class(sp.clim)
    ...
    biomod.sm <- BIOMOD_Modeling(bm.format = bmdat, #formatted biomod data
     ...
     # cv.k.df = as.data.frame(get_calib_lines(biomod.sm)) #uncomment if you want to save this as a data frame
    ...
    biomod.em <- try(biomod2::BIOMOD_EnsembleModeling(bm.mod = biomod.sm, #singles model output
                                                      ...
                                                      em.algo = c("EMmean", "EMcv"), #types of ensembles models to be computed (mean, and coefficient of variance) 
                                                      metric.select = c('TSS'),
    ...
    ###########################
    #These are the outputs we are interested in -- gives more flexibility in calculating the means and error for each cell
    ...
    # ),
    # error = function(e) { #This overrides the error message, if needed 
...
#Set vector of species
sp_names = sp[1:length(sp)] #can adjust to run in chunks, just change the starting number 
...
#save run time -- can comment out if this breaks 
