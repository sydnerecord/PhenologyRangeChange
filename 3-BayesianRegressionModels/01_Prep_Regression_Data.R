...
# Directories
#############
mwd = "E:/Liz/ESIIL_MacroPhenology"
L2 = file.path(mwd, "L2")
...
rang.comm.sp = rang %>%
  filter(species %in% comm.sp, !is.na(sd_diff)) %>%
  rename(rdiff = mean_diff, rsd = sd_diff) %>%
  arrange(species, latitude, longitude)
phe.comm.sp = phe %>%
  filter(species %in% comm.sp, !is.na(sd_diff)) %>%
  rename(pdiff = mean_diff, psd = sd_diff) %>%
  arrange(species, latitude, longitude)
#free up space
rm(phe, rang); gc()

#Merge the two (whichver one has more rows will be first)
